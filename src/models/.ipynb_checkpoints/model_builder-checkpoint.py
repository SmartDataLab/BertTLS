    ...
    def __init__(self, temp_dir, load_pretrained_bert, bert_config):
        ...
        if load_pretrained_bert:
            ...
            self.model = BertModel.from_pretrained(
                temp_dir + "bert-base-uncased.tar.gz", cache_dir=temp_dir
            )
        else:
...
class Summarizer(nn.Module):
    def __init__(self, args, device, load_pretrained_bert=False, bert_config=None):
        ...
        # if args.use_date:
        #     self.date_encoder =   # PositionalEncoding(
        #     args.dropout, self.bert.model.config.hidden_size
        # )
    ...
    def datetime2vec(self, date_list):
        date_encoder = self.encoder.pos_emb
        emb_list = [
            (
                date_encoder.pe[:, date.year]
                + date_encoder.pe[:, date.month]
                + date_encoder.pe[:, date.day]
            ).unsqueeze(1)
    ...
    def forward(
        self, x, segs, clss, mask, mask_cls, sentence_range=None, date_input=None
    ):
        ...
        if self.args.use_date:
            ...
            date_emb = torch.cat([self.datetime2vec(one) for one in date2day])
            sents_vec += date_emb
